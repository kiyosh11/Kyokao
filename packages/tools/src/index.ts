...
const clipped = (s, max = DEFAULT_MAX_OUTPUT) =>
...
export class CoreTools {
  ...
  async git(a) {
    if (
      ...
      return {
        content:
          'Only workspace-scoped, read-only git status/diff/log/show/branch arguments are allowed',
        isError: true,
      };
  ...
  async http(a) {
    ...
    for (let redirects = 0; redirects <= 5; redirects++) {
      ...
      if (response.status >= 300 && response.status < 400) {
        ...
        if (redirects === 5) return { content: 'HTTP redirect limit exceeded', isError: true };
      ...
      if (!reader) return { content: '', isError: !response.ok };
