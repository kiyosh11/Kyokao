...
/**
 * Permanent request failures cannot succeed when the same prompt is replayed
 * unchanged. Keeping them in the queue traps the workspace in `failed` and can
 * duplicate the user turn in a stateful backend.
 */
export function isPermanentPromptError(error) {
  ...
  return /^(Invalid|Unknown|Provider baseURL)|not available for chat|Safety limit reached|iteration limit/i.test(
    error.message,
  );
}
    ...
    try {
      ...
      if (!this.cancelled && !(error instanceof Error && error.name === 'AbortError')) {
        ...
        if (isPermanentPromptError(error)) {
          // The failed item was already shifted off the queue. Persist that
          // removal and stay interactive so later items continue instead of
          // stacking behind a request that cannot succeed unchanged.
          this.phase = 'idle';
          await this.changed();
          ...
          this.phase = 'failed';
          this.notify(message);
          this.options.emit('error', `${message} · use /queue retry`);
          return;
        }
      }
    } finally {
