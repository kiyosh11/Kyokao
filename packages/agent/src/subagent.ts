 ...
 */
export const SUBAGENT_DEFAULT_TOOLS = [
    'read_file',
    'list_files',
    'glob',
    'grep',
...
export class SubAgentTools {
    ...
    definitions() {
        return [
            spec('spawn_subagent', 'Delegate a scoped sub-task to a fresh sub-agent with a restricted tool set. The sub-agent investigates and returns its final answer; it does not see this conversation. Use for focused exploration (e.g. "find every caller of X") that would otherwise pollute the main transcript. By default the sub-agent can only read files and run read-only git; grant write/shell/http only when the sub-task needs them.', {
                prompt: {
                    type: 'string',
                ...
                tools: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'Optional subset of tool names to grant beyond the read-only default (read_file, list_files, glob, grep, git). Allowed additions: write_file, apply_patch, shell, http_get. Omit to keep the sub-agent read-only.',
                },
                maxIterations: {
                    type: 'integer',
                ...
                maxCostUsd: {
                    type: 'number',
                    description: 'Optional cost cap for this sub-agent in USD. Defaults to the smaller of 0.25 and the parent remaining budget.',
                },
            }, ['prompt']),
        ];
    }
    ...
    async spawn(args) {
        ...
        const maxIterations = typeof args.maxIterations === 'number' && args.maxIterations > 0
        ...
        const maxCostUsd = typeof args.maxCostUsd === 'number' && args.maxCostUsd > 0
