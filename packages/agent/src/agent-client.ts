...
export class AgentClientServer {
    ...
    constructor(backend, output, serverInfo = {
        name: 'kyokao',
        version: '0.5.5',
    }) {
    ...
    emitItem(turnId, kind, value) {
        ...
        switch (kind) {
            case 'assistant':
                this.write({ jsonrpc: '2.0', method: 'item/assistant', params: { ...params, text: String(value) } });
                break;
            case 'tool':
                this.write({ jsonrpc: '2.0', method: 'item/tool', params: { ...params, text: String(value) } });
                break;
            case 'tool-result':
                this.write({
                    jsonrpc: '2.0',
            ...
            case 'usage':
                ...
                break;
            case 'status':
                this.write({ jsonrpc: '2.0', method: 'item/status', params: { ...params, state: String(value) } });
                break;
            case 'error':
                this.write({
                    jsonrpc: '2.0',
    ...
    requireInit() {
        if (!this.initialized)
            throw new RpcError(ERR_INVALID_REQUEST, 'Server not initialized');
    }
}
