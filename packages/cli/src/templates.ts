...
export async function loadTemplates(
  dir?: string,
  builtins: AgentTemplate[] = BUILTIN_TEMPLATES,
...
function renderBareConditional(body: string, name: string, present: boolean): string {
  return body.replace(
    new RegExp(
      `\\{\\{#${escapeRegex(name)}\\}\\}([\\s\\S]*?)\\{\\{/${escapeRegex(name)}\\}\\}`,
      'g',
    ),
    (_, inner: string) => (present ? inner : ''),
  );
}
function renderConditionals(body: string, prefix: string, values: Record<string, unknown>): string {
  return body.replace(
    new RegExp(`\\{\\{#${prefix}\\.(\\w+)\\}\\}([\\s\\S]*?)\\{\\{/${prefix}\\.\\1\\}\\}`, 'g'),
    (_, key: string, inner: string) =>
      values[key] !== undefined && values[key] !== false && values[key] !== null ? inner : '',
  );
}
...
export function registerTemplates(program: Command, deps: TemplateDeps): void {
  for (const template of BUILTIN_TEMPLATES) {
    ...
    for (const flag of template.flags ?? []) cmd.option(flag);
    if (template.passthrough) cmd.allowUnknownOption(true);
    cmd.action(
      async (
        args: string[],
        context: {
