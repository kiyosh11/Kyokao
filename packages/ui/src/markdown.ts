...
const definitions: Record<string, LanguageDefinition> = {
  ...
  bash: {
    keywords: words(
      'if then else elif fi for while until do done case esac function in select time coproc local readonly declare export source',
    ),
    lineComments: ['#'],
    blockComments: [],
    quotes: ["'", '"', '`'],
  ...
  c: {
    keywords: words(
      'auto break case char const continue default do double else enum extern float for goto if inline int long register restrict return short signed sizeof static struct switch typedef union unsigned void volatile while class namespace public private protected template true false null nullptr',
    ),
    lineComments: ['//'],
    blockComments: [['/*', '*/']],
    quotes: ["'", '"'],
  ...
  html: {
    keywords: words(
      'html head body script style div span main section article nav a p h1 h2 h3 ul ol li',
    ),
    lineComments: [],
    blockComments: [['<!--', '-->']],
    quotes: ["'", '"'],
  ...
  css: {
    keywords: words(
      'important inherit initial unset revert none block inline flex grid relative absolute fixed sticky auto solid transparent',
    ),
    lineComments: ['//'],
    blockComments: [['/*', '*/']],
    quotes: ["'", '"'],
  ...
  sql: {
    keywords: words(
      'select from where join inner left right full on group by order having limit offset insert into values update set delete create alter drop table index view distinct union all as and or not null is in exists between like case when then else end primary key foreign references',
    ),
    lineComments: ['--'],
    blockComments: [['/*', '*/']],
    quotes: ["'", '"', '`'],
...
export class CodeRenderer {
  ...
  render(source: string, language = ''): string {
    ...
    if (name === 'diff' || name === 'patch') return this.diff(source);
    ...
    while (index < source.length) {
      ...
      if (isIdentifierStart(source[index]!)) {
        let next = index + 1;
        while (next < source.length && isIdentifierPart(source[next]!)) next++;
        const value = source.slice(index, next);
        const lower = value.toLowerCase();
        const after = source.slice(next).match(/^\s*(.)/)?.[1];
        const token: CodeToken = ['true', 'false', 'null', 'none', 'nil', 'undefined'].includes(
          lower,
        )
          ? 'booleanNull'
          : definition.keywords.has(value) || definition.keywords.has(lower)
            ? 'keyword'
            : after === '(' ||
                /^(?:class|interface|type|struct|enum)$/.test(
                  source.slice(0, index).trim().split(/\s+/).at(-1) ?? '',
                )
              ? 'functionType'
...
export class MarkdownRenderer {
  ...
  render(source: string): string {
    ...
    for (let index = 0; index < lines.length; index++) {
      ...
      if (open) {
        ...
        if (closed !== undefined) result += this.context.code('fenceLabel', closed);
        continue;
      }
  ...
  private markdownLine(line: string): string {
    ...
    if (heading)
      return (
        this.context.tui('brand', heading[1]!) + this.context.tui('brand', this.inline(heading[2]!))
      );
    const quote = line.match(/^(\s*>+\s?)(.*)$/s);
    if (quote)
      return (
        this.context.tui('border', quote[1]!) + this.context.tui('muted', this.inline(quote[2]!))
      );
    const list = line.match(/^(\s*)([-+*]|\d+[.)])(\s+)(.*)$/s);
    if (list)
      return (
        list[1]! + this.context.tui('inputAccent', list[2]!) + list[3]! + this.inline(list[4]!)
      );
  ...
  private inline(source: string): string {
    ...
    while (index < source.length) {
      ...
      let next = index + 1;
      while (next < source.length && !'`[*_'.includes(source[next]!)) next++;
...
export class MarkdownStreamRenderer {
  private pending = '';
  private fence?: { marker: string; size: number; language: string };
  ...
  private line(line: string): string {
    ...
    const open = line.match(/^(\s*)(`{3,}|~{3,})([^`\n~]*)/);
    if (!open) return this.markdown.render(line);
    this.fence = {
      marker: open[2]![0]!,
      size: open[2]!.length,
